...
def mqtt_check_and_reconnect(client):
    while True:
        try:
            ...
            time.sleep(5)  # Wait before retrying to avoid overwhelming the broker
...
def main():
    ...
    wlan.active(True)
    wlan.config(dhcp_hostname=f"esp32_{machine_id}")
    ...
    try:
        ...
        print('subscribed to mqtt for led')
    except:
        mqtt_check_and_reconnect(client)
    
    while not published_config:
            publish_config_payloads(client, machine_id, temp_config_payload, humidity_config_payload, led_config_payload)
    
        
        
    published_led_state=False
    
    led_state = "ON" if p2.value() == 1 else "OFF"
    state_topic = f"homeassistant/switch/esp32_{machine_id}/led/state"
    while not published_led_state:
        try:
            client.publish(state_topic, led_state)
            published_led_state = True
        except:            
            mqtt_check_and_reconnect(client)
    ...
    while True:
        if not wlan.isconnected():
            ...
            connect_to_wifi(wlan)
        if not client.sock:
            mqtt_check_and_reconnect(client)  # Reconnect MQTT if needed

        try:
            client.check_msg()
        except:
            pass# Check and reconnect MQTT if needed
        t, p, h = bme.read_compensated_data()
        temperature = t / 100
        humidity = h / 1024
        current_seconds = utime.localtime()[5]
        if current_seconds==20 or current_seconds==50:
            publish_sensor_data(client, machine_id, temperature, round(humidity,2))
            time.sleep(1)
